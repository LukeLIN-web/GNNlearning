...
https://youtu.be/hlcPz7wuf3M?si=lQou_ZgOkHaG4z6K
...
```
fuse_modules 会fuse conv和batch norm,bn1的weight fuse_modules_qat会保持 batch norm, 
```
...
**在fx进行trace的时候没有报错，但是训练过程中报错，且报错信息是fx的内部表示，无法定位至模型搭建代码**

可以尝试将qconfig逐个设置为None排查。这样速度比较慢，目前我也没找到合适的方法。
